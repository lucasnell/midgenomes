...
if [ ! -f /staging/lnell/dna/trimmed/${READS1}.gz ]; then
    echo "/staging/lnell/dna/trimmed/${READS1}.gz does not exist!" 1>&2
    exit 111
fi
if [ ! -f /staging/lnell/dna/trimmed/${READS2}.gz ]; then
    echo "/staging/lnell/dna/trimmed/${READS2}.gz does not exist!" 1>&2
    exit 222
fi
...
mv ${OUT_BAM} /staging/lnell/dna/bwa/


rm ${READS1} ${READS2} ${GENOME}

cd ..

tar -czf ${OUT_DIR}.tar.gz ${OUT_DIR}
mv ${OUT_DIR}.tar.gz /staging/lnell/dna/bwa/
