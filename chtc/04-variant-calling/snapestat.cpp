...
/*
 Highly modified version of npstat by Lucas A. Nell.

 Usage:
    snapestat -n samplesize -l windowlength -s file.snp file.pileup

 It outputs the following (columns 1,2,4-10 in original npstat):
    1. window number
    2. number of bases covered by sequences
    3. average read depth
    4. number of segregating sites S
    5. Watterson estimator of θ
    6. Tajima’s Π estimator of heterozygosity
    7. Tajima’s D
    8. variance of the number of segregating sites
    9. variance of the Watterson estimator

 IMPORTANT NOTE: This version is designed to work with output from a
 SNP caller like SNAPE-pooled,
 so it always assumes the following original npstat arguments:
 `-maxcov 10000 -nolowfreq 0 -mincov 4 -minqual 0`

 Argument parsing is from https://stackoverflow.com/a/868894

 */

/* Compile with
 g++ -std=c++11 -O3 -o snapestat snapestat.cpp
*/

/* Arguments:
 -n samplesize : haploid sample size
 -l windowlength : window length
 -s file.snp : consider SNPs only if present in file.snp\
*/
...
public:
    ...
    /// @author LAN
    const std::string& getLastOption() const {
        std::vector<std::string>::const_iterator itr = this->tokens.end();
        --itr;
        return *itr;
    }
    /// @author iain
...
int main(int argc, char **argv){
    ...
    if(input.cmdOptionExists("-h")){
        ...
        std::cout << "    npstat [options] file.pileup" << std::endl;
        std::cout << "   -n samplesize : haploid sample size" << std::endl;
        std::cout << "   -l windowlength : window length" << std::endl;
        std::cout << "   -s file.snp : consider SNPs only " <<
            "if present in file.snp" << std::endl;
        return 0;
    }
    double hn, wl;
    ...
    if (!n_str.empty()) {
        ...
        std::cerr << "-n option not provided!" << std::endl;
        return 1;
    }
    const std::string& l_str = input.getCmdOption("-l");
    if (!n_str.empty()) {
        wl = std::strtod(l_str);
    } else {
        std::cerr << "-l option not provided!" << std::endl;
        return 1;
    }
    const std::string& snp_name = input.getCmdOption("-s");
    if (snp_name.empty()) {
        std::cerr << "-s option not provided!" << std::endl;
        return 1;
    }
    const std::string& pileup_name = input.getLastOption();
    if (pileup_name.empty()) {
        std::cerr << "pileup file not provided!" << std::endl;
        return 1;
    }
