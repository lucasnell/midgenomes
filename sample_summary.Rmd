...
myvatn_df <- readOGR(dsn = "~/Box Sync/2020/midges/shapefiles/myvatn", 
    ...
    tidy()
trap_df <- read_csv("~/Box Sync/2020/midges/trap_locations.csv") %>% 
    rename(long = lon) %>% 
    to_utm() %>% 
...
archive_df <- read_excel("~/Box Sync/2020/midges/tany_from_archive.xlsx",
                        sheet = "data-entry") %>%
...
tany_pop_df <- read_excel("~/Box Sync/2020/funding/ASN/Chirisl_Tany_analysis.xlsx",
...
archive_ts_p <- archive_ts_df %>%
    ...
    geom_point(aes(shape = using, color = site), na.rm = TRUE) +
    ...
    scale_color_manual(NULL, values = archive_pal, guide = FALSE) +
